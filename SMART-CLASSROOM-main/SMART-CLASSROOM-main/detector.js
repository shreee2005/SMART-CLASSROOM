...
function getEyeAspectRatio(eyeLandmarks) {
  ...
  const h1 = distance(eyeLandmarks[0], eyeLandmarks[3]);
  return (v1 + v2) / (2 * h1);
}
function estimateHeadYaw(landmarks) {
  const noseTip = landmarks.positions[30];  // nose tip
  const chin = landmarks.positions[8];      // chin center
  const yawAngle = Math.atan2(chin.x - noseTip.x, chin.y - noseTip.y) * 180 / Math.PI;
  return yawAngle;
}